<template>
    <v-dialog v-model="dialog" width="550">
        <v-card class="pa-4">
            ...
            <v-card-text>
                ...
                <v-text-field
                    v-model.trim="
                        configOptions['advanced-options']['tile-size']
                    "
                    maxlength="20"
                    label="tile-size"
                ...
                <v-text-field
                    v-model.trim="
                        configOptions['advanced-options']['load-proc-save']
                    "
                    maxlength="20"
    ...
    <div v-if="!isAdvancedOptionsEmpty">
        <p v-if="configOptions['advanced-options']?.tta">
            <strong>TTA: </strong
            >{{ configOptions["advanced-options"]?.tta ? "Active" : "" }}
        </p>
        <p v-if="configOptions['advanced-options']?.['gpu-id']">
            <strong>gpu-id: </strong
            >{{ configOptions["advanced-options"]?.["gpu-id"] }}
        </p>
...
const rules = {
    gpuId: (val: string) => {
        ...
        return (
            pattern.test(val) ||
            "gpu-id must be empty, a integer or a list of integers separated by commas"
        );
    },
};
...
const isAdvancedOptionsEmpty = computed(() => {
    return (
        !configOptions.value["advanced-options"]?.["gpu-id"] &&
...
onMounted(async () => {
    try {
        ...
        sendTauriNotification("Error", "Error loading configuration file");
    }
});
...
async function writeConfiguration() {
    try {
        ...
        sendTauriNotification("Error", "Error writing configuration file");
    }
}
