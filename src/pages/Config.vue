<template>
  <div class="container">
    ...
    <v-select class="select-fields ml-5" label="Default Upscale Type" v-model="options['default-upscale-type']"
      variant="solo" density="comfortable" :items="optionsUpscaleType" item-title="text" item-value="value" hide-details
      @update:modelValue="write_configuration()" />
    ...
    <div class="max-size-option">
      ...
      <v-select class="select-fields ml-5" label="Size in MB" v-model="options['max-preview-upscale-size']"
        variant="solo" density="comfortable" :items="optionsPreviewMaxSize" hide-details
        @update:modelValue="write_configuration()" />
    </div>
  </div>
</template>
